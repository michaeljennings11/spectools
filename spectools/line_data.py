...
def load_elementDataFrame(element):
    ...
    try:
        df = pd.read_csv(file, usecols=np.arange(0,ncols+1)).replace(['=','"',r'\[',r'\]',r'\(',r'\)',r'\+x',r'\?','&dagger',';'],\
                                                                  ['','','','','','','','','',''], regex=True).infer_objects()
        df[[wave_label,'Aki(s^-1)','fik','Ei(eV)','Ek(eV)']] =\
            df[[wave_label,'Aki(s^-1)','fik','Ei(eV)','Ek(eV)']].astype('float')
        ...
        raise ValueError(f"{element} file not found in NIST_lines directory.\n\
...
def get_Elevels(line):
    ...
    df_group = df.loc[(df["terml"]==iterm) & (df["termu"]==kterm) & (df["confl"]==iconf) & (df["confu"]==kconf)]
    return df_group
...
#         wave_label = 'obs_wl_vac(A)'
    ...
    for i in range(nlines):
        grid[idx_up[i]:idx_down[i],idx_lines[i]] = '|' # fill level transition arrow bars
